...
let RolesGuard = class RolesGuard {
    ...
    canActivate(context) {
        const allowInactiveAccount = this.reflector.getAllAndOverride('allowInactiveAccount', [
            context.getHandler(),
            context.getClass(),
        ]) || false;
        ...
        if (!user?.role) {
            throw new common_1.ForbiddenException('Role not found on user');
        }
        ...
        if (!requiredRoles.some(role => role.toLowerCase().trim() === userRole)) {
            throw new common_1.ForbiddenException(`Insufficient role: ${userRole}`);
        }
        return true;
    }
};
